...
async function init() {
    try {
        allProducts = await (0, _api.fetchProducts)();
        (0, _ui.displayProducts)(allProducts);
    } catch (error) {
...
function applyFilters() {
    ...
    const maxPrice = parseFloat(document.getElementById('max-price-filter').value);
    const sortOption = document.getElementById('sort-filter').value;
    if (category) filteredProducts = (0, _filter.filterByCategory)(category, filteredProducts);
    if (!isNaN(maxPrice)) filteredProducts = (0, _filter.filterByMaxPrice)(maxPrice, filteredProducts);
    switch(sortOption){
        case 'price-asc':
            filteredProducts = (0, _filter.sortByPriceAscending)(filteredProducts);
            break;
        case 'price-desc':
            filteredProducts = (0, _filter.sortByPriceDescending)(filteredProducts);
            break;
        case 'rating-asc':
            filteredProducts = (0, _filter.sortByRatingAscending)(filteredProducts);
            break;
        case 'rating-desc':
            filteredProducts = (0, _filter.sortByRatingDescending)(filteredProducts);
            break;
    }
    (0, _ui.displayProducts)(filteredProducts);
}
...
parcelHelpers.export(exports, "sortByPriceAscending", ()=>sortByPriceAscending);
parcelHelpers.export(exports, "sortByPriceDescending", ()=>sortByPriceDescending);
parcelHelpers.export(exports, "sortByRatingAscending", ()=>sortByRatingAscending);
parcelHelpers.export(exports, "sortByRatingDescending", ()=>sortByRatingDescending);
function filterByCategory(category, products) {
    return products.filter((product)=>product.category === category);
}
function filterByMaxPrice(maxPrice, products) {
...
function sortByRatingAscending(products) {
    return products.slice().sort((a, b)=>a.rating - b.rating);
}
function sortByRatingDescending(products) {
    return products.slice().sort((a, b)=>b.rating - a.rating);
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["7Js9b","gH3Lb"], "gH3Lb", "parcelRequire85b6")

//# sourceMappingURL=FE24-JS2-MP2-Matti-Walli.34df32e0.js.map
